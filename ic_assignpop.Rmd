...
The K-fold is now done, with three assignment tests conplete! The data has been sent to a folder called "morphResultFolder_KF". We can now visualize our data with a membership probabiltiy plot.
...
Now we can run the Monte-Carlo for the morhpometric data.
...
Our dataset has now been reduced. As shown by the output in the console, 81 alleles were removed and 9 loci remain. We wil now run the K-fold test for the genetic data.
...
Great! Three assignment tests were completed and now we can visualize it similarly to the morphometric data with a membership probablity plot.
...
#Now, you will be prompted to choose an output style just like the morphometric data. We challenge you to run this four times to see all the outputs, but for now just type "3" for the "Seperated by fold" option.
```
...
The Monte-Carlo is done with three assignment tests complete! Simialrly to the morhpometric data, we can calulate the assignment accuracy and plot it!
...
Identifying important loci is important because if you were using these samples for an actual experiment, it could help you identify loci that might be associated with functional genes. It coud also help to reduce time and cost of preparing samples.

This is being based off of the Fst values. Fst values, or the fixation index, are a measure of population differentiation due to genetic strucutre. The values ramge from 0 to 1, w 0 meaning the samples are not different, and with 1 being very different and no alleles in common. 
